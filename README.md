...
- Use the [Gradle](https://gradle.org/) build tool for building the Java code. See [app/build.gradle](app/build.gradle) for details.
- Create a Docker container image from the [Dockerfile](Dockerfile), pushed to [DockerHub](https://hub.docker.com/r/informaticsmatters/squonk2-cdk).
- Write the Squonk2 job definition, including a [jote](https://github.com/InformaticsMatters/data-manager-job-tester) test. See [data-manager/cdk.yaml](data-manager/cdk.yaml).


## Building

You need **JDK 21** and Docker. Gradle itself comes from the wrapper, so
`./gradlew` is all you need; it does not download a JDK for you, and the build
declares a Java 21 toolchain, so a JDK 21 has to be on the machine (or on
...
```shell
$ docker build -f Dockerfile . -t informaticsmatters/squonk2-cdk:latest
```
The Dockerfile is a two-stage build: it runs `./gradlew installDist` in a JDK
image and copies the result onto a JRE, so it needs nothing installed locally
beyond Docker. The image used to be generated by the bmuschko
`docker-java-application` Gradle plugin; it is a plain Dockerfile now, which is
what CI builds.
...
**Running the Job tests**:

[jote] runs the tests declared in the Job Definition against a locally built
image. The manifest is not at jote's default path, so name it:

```shell
$ docker build -f Dockerfile . -t informaticsmatters/squonk2-cdk:ci
$ jote -m manifest-cdk.yaml --image-tag ci
```

`--image-tag` overrides the tag in the Job Definition, so a new image can be
tested without editing the YAML.

[jote]: https://github.com/InformaticsMatters/data-manager-job-tester
